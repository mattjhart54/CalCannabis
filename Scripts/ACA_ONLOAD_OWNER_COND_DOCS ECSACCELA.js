...
//	loadASITables4ACA(capId);
	...
	uploadedDocs = new Array();
			showMessage = true;
		message += "<div class='docList'><span class='fontbold font14px ACA_Title_Color'>Entering the for: loop of submittedDocList</span><br>";
	for (var i in submittedDocList ){
		uploadedDocs[submittedDocList[i].getDocCategory()] = true;
		message += "loop:" + i;
	}

	message += "r length: " + r.length + " showList: " + showList + " addTableRows: " + addTableRows + " length of Cond Table: " + conditionTable.length; 
		message += "</div>";
	if (r.length > 0 && (showList || addTableRows)) {
		for (x in r) { 
		message += "<div><span> x = : " + x + "</span></div>";
...
if(uploadedDocs[r[x].document] == undefined) {
				showMessage = true; 
				if (!docsMissing)  {
					comment("<div class='docList'><span class='fontbold font14px ACA_Title_Color'>IN A: The following documents are required based on the information you have provided:" + uploadedDocs[r[x].document] + " </span><ol>"); 	
					docsMissing = true; 
				}
				...
				if (dr && ccr.length > 0 && showList && publicDisplayCond) {
					message += "<li><span> IN B:" + dr + "</span>: " + publicDisplayCond.getPublicDisplayMessage() + "</li>";
				}
				if (dr && ccr.length > 0 && addConditions && !appHasCondition(conditionType,null,dr,null)) {
					addStdCondition(conditionType,dr);
				}
				message += "<span> IN C: dr " + dr + " ccrlength: " + ccr.length + " addTableRows: " + addTableRows+ "</span>";
				...
				//	conditionTable.push(tblRow);
					message += "<div><span> IN: D not Pushing into table " +tblRow["Document Type"] + ": " + tblRow["Document Description"] + "</span></div>" + br;
	...
	message += "<div><span> IN: ELSE" + dr +  "<div><span>";}//moved to 
