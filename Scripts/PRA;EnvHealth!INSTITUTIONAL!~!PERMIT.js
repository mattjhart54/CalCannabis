...
if(renewBal <= 0 && feeExists("060") )
{
	var expDate = aa.expiration.getLicensesByCapID(capId).getOutput().getExpDateString().replaceAll('-','/');	closeTask("Permit Status"," Renewed","Renewed on "+ sysDate +" via PRA ",null); 
	...
	if(doPermitIssuedDate){
		...
		{ logDebug("Warning: Failed to update the renewal info, please define the permit type in the Standard Choice EH_RENEWAL_INTERVAL._Java"); }
	}
	if(doFixedDate){
		var fixedDates= lookup("EH_RENEWAL_FIXED_DATE", appTypeString);
		...
		{ logDebug("Warning: Failed to update the renewal info, please define the permit type in the Standard Choice  EH_RENEWAL_FIXED_DATE._JAVA"); }
	}
}
