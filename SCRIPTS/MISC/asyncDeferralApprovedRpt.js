		...
		for (iNum in iList)	{
			...
			for (fNum in fList) {
				...
				if (fList[fNum].getFeeSeqNbr() == feeSeqNbr) {
					...
					feeAmt = fList[fNum].getFee();
logDebug("invNbr " + invNbr + " Fee Amount " + feeAmt);							
				}
			}
		}		
	}
	...
	for (ff in feeObjArr) {
		if (feeObjArr[ff].getFeeitemStatus() == "INVOICED") {
			...
			var pfResult = aa.finance.getPaymentFeeItems(tmpID, null);
			if (pfResult.getSuccess()) {
				var pfObj = pfResult.getOutput();
				for (ij in pfObj) {
					if (feeObjArr[ff].getFeeSeqNbr() == pfObj[ij].getFeeSeqNbr()) {
						amtPaid += pfObj[ij].getFeeAllocation();
						logDebug("fee " + feeObjArr[ff].getFeeCod() + " status " + feeObjArr[ff].getFeeitemStatus() + " amt " + feeObjArr[ff].getFee() + " Paid " + pfObj[ij].getFeeAllocation());
					}
				}
			}
