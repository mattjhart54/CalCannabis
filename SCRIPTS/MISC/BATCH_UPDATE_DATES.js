...
//		if(altId != "LCA19-0000004") continue;
		var taskResult = aa.workflow.getHistory(capId);
		if (taskResult.getSuccess()) {
			var taskArr = taskResult.getOutput();
			var licTask = "Final Review";
			var adminTask = "Administrative Manager Review";
			var adminDue = null;
			var scienceTask = "Science Manager Review";
			var scienceDue = null;
			for (xx in taskArr) {
				if (taskArr[xx].getTaskDescription().equals(licTask) && (taskArr[xx].getDisposition().equals("Approved for Provisional License") || taskArr[xx].getDisposition().equals("Approved for Annual License"))) {
					var taskDate = taskArr[xx].getDispositionDate();
					if (taskDate) {
						dueDate = nextWorkDay(dateAdd(jsDateToASIDate(new Date(taskDate.getMonth() + "/" + taskDate.getDayOfMonth() + "/" + taskDate.getYear())),89));
						logDebug("Record " + altId + " Task " + taskArr[xx].getTaskDescription() + " status " + taskArr[xx].getDisposition() + " Date " + dueDate);
						licDateUpdt++;
						editAppSpecific("License Fee Due",dueDate);
					}
				}
				if (taskArr[xx].getTaskDescription().equals(adminTask) && taskArr[xx].getDisposition().equals("Deficiency Letter Sent")) {
					var taskDate = taskArr[xx].getDispositionDate();
					if (taskDate) {
						dueDate = jsDateToASIDate(new Date(taskDate.getMonth() + "/" + taskDate.getDayOfMonth() + "/" + taskDate.getYear()));
						logDebug("Record " + altId + " Task " + taskArr[xx].getTaskDescription() + " status " + taskArr[xx].getDisposition() + " Date " + dueDate);
						adminDateUpdt++
						editAppSpecific("Admin Deficiency Letter Sent",dueDate);
					}
				}
				if (taskArr[xx].getTaskDescription().equals(scienceTask) && taskArr[xx].getDisposition().equals("Deficiency Letter Sent")) {
					var taskDate = taskArr[xx].getDispositionDate();
					if (taskDate) {
						dueDate = jsDateToASIDate(new Date(taskDate.getMonth() + "/" + taskDate.getDayOfMonth() + "/" + taskDate.getYear()));
						logDebug("Record " + altId + " Task " + taskArr[xx].getTaskDescription() + " status " + taskArr[xx].getDisposition() + " Date " + dueDate);
						sciDateUpdt++
						editAppSpecific("Science Deficiency Letter Sent",dueDate);
					}
				}							
			}
